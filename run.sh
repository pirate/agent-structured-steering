...
if [[ "$POC_DEMO" -eq 1 ]]; then
  ...
  python3 -c 'import json,sys; print(json.dumps({"threadId":"demo","statePath":sys.argv[1],"eventsPath":sys.argv[2],"source":"demo"}))' \
    "$POC_DEMO_DIR/state.json" "$POC_DEMO_DIR/events.jsonl" >"$POC_ACTIVE"
else
  POC_THREAD_ARGS=()
